...
function complete(elements) {
    ...
    var newContent = '';
    var comments = row[0].querySelector('p>strong:nth-child(2)');
    var dudes = elements.length;

    elements.forEach(function(data) {
        newContent = newContent + '<article><h4><a href="dude/finn-the-human">' + data.who + '</a></h4><p>' + data.wat + '</p><footer class="comments">' + data.comments + '</footer></article>';
        comments = comments + data.comments;
    });

    row[0].innerHTML = newContent;
    row[1].innerHTML = '<p>number of dudes: <strong>' + dudes + '</strong><br>number of comments: <strong>' + comments + '</strong></p>';
}
